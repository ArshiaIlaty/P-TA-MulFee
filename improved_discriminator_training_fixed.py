...
from fixed_sentence_discriminator import fix_sentence_discriminator_in_system
...
class ImprovedDiscriminatorTrainingFixed:
    """
    Improved discriminator training with properly fixed sentence discriminator
    """
    ...
    def __init__(self, hierarchical_discriminators, device="cuda", checkpoint_dir="checkpoints"):
        ...
        self.sentence_fix_successful = False
        
        # Create checkpoint directory
        os.makedirs(checkpoint_dir, exist_ok=True)
        
        # Fix sentence discriminator
        self.sentence_fix_successful = fix_sentence_discriminator_in_system(
            self.discriminators, device
        )
        
        if self.sentence_fix_successful:
            logger.info("✅ Sentence discriminator successfully fixed!")
        else:
            logger.warning("⚠️ Sentence discriminator fix failed, continuing with original")
    ...
    def save_checkpoint(self, generator, cycle, best_generator_loss, training_history):
        ...
        checkpoint = {
            ...
            'sentence_fix_successful': self.sentence_fix_successful
        }
    ...
    def load_checkpoint(self, generator, checkpoint_path):
        ...
        if os.path.exists(checkpoint_path):
            ...
            self.sentence_fix_successful = checkpoint.get('sentence_fix_successful', False)
            
            logger.info(f"📂 Checkpoint loaded from {checkpoint_path}")
            logger.info(f"🔄 Resuming from cycle {checkpoint['cycle']} with best loss {checkpoint['best_generator_loss']:.4f}")
            logger.info(f"🔧 Sentence fix status: {'✅ Successful' if self.sentence_fix_successful else '❌ Failed'}")
    ...
    def train_discriminators_improved(
        ...
        """Improved discriminator training with fixed sentence discriminator"""
    ...
    def train_sentence_discriminator_batch(self, texts, labels):
        """Train sentence discriminator on a batch with proper error handling"""
        total_loss = 0
        num_sentences = 0
            ...
            for sentence in sentences:
                if " is " in sentence:
                    try:
                        ...
                        if hasattr(self.discriminators, 'sentence_tokenizer'):
                            tokenizer = self.discriminators.sentence_tokenizer
                        else:
                            tokenizer = self.discriminators.tokenizer
                        ...
                        if hasattr(self.discriminators.sentence_discriminator, 'forward'):
                            ...
                            prediction = self.discriminators.sentence_discriminator(encoding["input_ids"])
    ...
    def train_generator_with_feedback(self, generator, real_data, epochs=2, batch_size=16):
        ...
        # Balanced weights - adjust based on sentence fix status
        if self.sentence_fix_successful:
            balanced_weights = {
                'token': 0.2,
                'sentence': 0.2,  # Enable with fix
            ...
            balanced_weights = {
                'token': 0.2,
                'sentence': 0.0,  # Disable without fix
    ...
    def plot_impact_analysis_fixed(self, save_path='impact_analysis_fixed.png'):
        """Fixed impact analysis plot with comprehensive metrics"""
        ...
        logger.info(f"📊 Fixed impact analysis plot saved to {save_path}")
...
def main():
    """Example usage of fixed improved discriminator training"""
    # Initialize wandb
    try:
        wandb.init(project="improved-discriminator-training", name="heloc-fixed-sentence")
    except:
    ...
    # Initialize improved training framework with fixed sentence discriminator
    logger.info("🚀 Initializing improved training framework with fixed sentence discriminator...")
    improved_trainer = ImprovedDiscriminatorTrainingFixed(hierarchical_discriminators, device)
    ...
    # Generate fixed impact analysis plot
    logger.info("📊 Generating fixed impact analysis plot...")
    improved_trainer.plot_impact_analysis_fixed('heloc_impact_analysis_fixed.png')
    logger.info("✅ Fixed impact analysis plot saved to heloc_impact_analysis_fixed.png")
    ...
    results = {
        ...
        'training_history': training_history,
        'sentence_fix_successful': improved_trainer.sentence_fix_successful
    }
    
    with open('training_results_fixed.json', 'w') as f:
        json.dump(results, f, indent=2, default=str)
    logger.info("✅ Comprehensive results saved to training_results_fixed.json")
    
    logger.info("🎉 Fixed improved discriminator training completed!")
