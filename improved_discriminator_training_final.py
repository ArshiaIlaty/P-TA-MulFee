...
class ImprovedDiscriminatorTrainingFinal:
    """
    Final improved discriminator training with sentence discriminator fix and checkpointing
    """
    ...
    def __init__(self, hierarchical_discriminators, device="cuda", checkpoint_dir="checkpoints"):
        ...
        self.discriminator_learning_rate = 5e-4  # Increased from 1e-4
        self.generator_learning_rate = 1e-5      # Reduced to balance training
        
        # Create checkpoint directory
        os.makedirs(checkpoint_dir, exist_ok=True)
        
        # Fix sentence discriminator with BERT
        self.fix_sentence_discriminator()
        
        # Enable gradient checkpointing for memory efficiency
        self.enable_gradient_checkpointing()
        
    def fix_sentence_discriminator(self):
        """
        Replace GPT-2 sentence discriminator with BERT for better classification
        """
        logger.info("Fixing sentence discriminator with BERT...")
        ...
        try:
            ...
            # Create a custom sentence discriminator that properly handles DistilBERT output
            class FixedSentenceDiscriminator(torch.nn.Module):
                def __init__(self, model_name, device):
                    super().__init__()
                    self.bert = AutoModelForSequenceClassification.from_pretrained(
                        model_name, num_labels=1
                    ).to(device)
                    # Add a proper output layer for binary classification with better scaling
                    self.classifier = torch.nn.Sequential(
                        torch.nn.Linear(1, 64),      # Expand to hidden layer
                        torch.nn.ReLU(),             # Activation
                        torch.nn.Dropout(0.1),       # Light dropout
                        torch.nn.Linear(64, 1),      # Back to single output
                        torch.nn.Sigmoid()           # Sigmoid to ensure output is in [0,1]
                    ).to(device)
                ...
                def _init_weights(self):
                    """Initialize classifier weights to prevent extreme values"""
                ...
                def forward(self, input_ids, attention_mask=None):
                    ...
                    # Use the logits and apply our classifier
            ...
            # Create the fixed sentence discriminator
            sentence_model = FixedSentenceDiscriminator(sentence_model_name, self.device)
            ...
            # Update optimizer with improved learning rate
            self.discriminators.optimizers["sentence"] = torch.optim.AdamW(
                sentence_model.parameters(), lr=self.discriminator_learning_rate
            )
            
            logger.info("Sentence discriminator successfully replaced with fixed DistilBERT")
        
        except Exception as e:
            logger.warning(f"Failed to replace sentence discriminator: {e}")
    ...
    def enable_gradient_checkpointing(self):
        ...
        try:
            ...
            if hasattr(self.discriminators.sentence_discriminator, 'gradient_checkpointing_enable'):
                try:
                    self.discriminators.sentence_discriminator.gradient_checkpointing_enable()
                    logger.info("Enabled gradient checkpointing on sentence discriminator")
                except ValueError:
                    logger.info("Sentence discriminator does not support gradient checkpointing")
            
            if hasattr(self.discriminators.row_discriminator, 'gradient_checkpointing_enable'):
                try:
                    self.discriminators.row_discriminator.gradient_checkpointing_enable()
                    logger.info("Enabled gradient checkpointing on row discriminator")
                except ValueError:
                    logger.info("Row discriminator does not support gradient checkpointing")
    ...
    def alternating_training(
        ...
        real_data, 
        synthetic_data, 
        num_cycles=10,  # Increased to 10 cycles
        discriminator_epochs=3,  # Increased discriminator training
        generator_epochs=2,
        ...
        for cycle in range(start_cycle, num_cycles):
            ...
            # Clear memory before each cycle
            self.clear_memory()
            ...
            # Clear memory between discriminator and generator training
            self.clear_memory()
    ...
    def train_discriminators_improved(
        ...
        """
        Improved discriminator training with better sentence discriminator
        """
        ...
        for epoch in range(epochs):
            ...
            if torch.cuda.is_available():
                ...
                logger.info(f"Epoch {epoch+1} - GPU Memory: {memory_allocated:.2f}GB allocated, {memory_reserved:.2f}GB reserved")
            epoch_losses = []
            ...
            for batch_idx in range(num_batches):
                ...
                # Train sentence discriminator (now with BERT)
                sentence_loss = self.train_sentence_discriminator_batch_v2(batch_texts, batch_labels)
    ...
    def train_sentence_discriminator_batch_v2(self, texts, labels):
            ...
            for sentence in sentences:
                if " is " in sentence:
                    try:
                        ...
                        if hasattr(self.discriminators.sentence_discriminator, 'get_input_embeddings'):
                            vocab_size = self.discriminators.sentence_discriminator.get_input_embeddings().num_embeddings
                            if encoding["input_ids"].max() >= vocab_size:
                                logger.warning(f"Token ID {encoding['input_ids'].max()} exceeds vocab size {vocab_size}")
                                continue
                        
                        self.discriminators.optimizers["sentence"].zero_grad()
                        
                        # Handle the fixed sentence discriminator
                        if hasattr(self.discriminators.sentence_discriminator, 'classifier'):
                            # Our fixed discriminator returns sigmoid output
                        ...
                        # Add regularization to prevent discriminator degradation
                        l2_reg = 0.01
                        for param in self.discriminators.sentence_discriminator.parameters():
    ...
    def train_generator_with_feedback(self, generator, real_data, epochs=2, batch_size=16):
        ...
        generator.train()
        optimizer = torch.optim.AdamW(generator.parameters(), lr=self.generator_learning_rate)
        epoch_losses = []
        
        # Balanced weights - keep sentence discriminator enabled
        balanced_weights = {
            'token': 0.2,
            'sentence': 0.2,  # Keep enabled with BERT fix
        ...
        for epoch in range(epochs):
            ...
            for batch_idx in range(num_batches):
                ...
                # Update generator - accumulate gradients from all samples
                optimizer.zero_grad()
                ...
                if wandb.run is not None:
                    ...
                    avg_token_loss = np.mean([1 - feedback["token"] for feedback in feedbacks]) if feedbacks else 0
                    avg_sentence_loss = np.mean([1 - feedback["sentence"] for feedback in feedbacks]) if feedbacks else 0
                    avg_row_loss = np.mean([1 - feedback["row"] for feedback in feedbacks]) if feedbacks else 0
                    avg_feature_loss = np.mean([1 - np.mean(list(feedback["features"].values())) for feedback in feedbacks]) if feedbacks else 0
                    
                    wandb.log({
                        f"generator_epoch_{epoch+1}_batch_{batch_idx}_loss": avg_batch_loss,
                        f"generator_epoch_{epoch+1}_batch_{batch_idx}_token_loss": avg_token_loss,
                        f"generator_epoch_{epoch+1}_batch_{batch_idx}_sentence_loss": avg_sentence_loss,
                        f"generator_epoch_{epoch+1}_batch_{batch_idx}_row_loss": avg_row_loss,
                        f"generator_epoch_{epoch+1}_batch_{batch_idx}_feature_loss": avg_feature_loss
                    })
    ...
    def plot_impact_analysis_final(self, save_path='impact_analysis_final.png'):
        """
        Final impact analysis plot with comprehensive metrics
        """
        ...
        logger.info(f"Final impact analysis plot saved to {save_path}")
...
def main():
    """
    Example usage of final improved discriminator training
    """
    # Initialize wandb
    try:
        wandb.init(project="improved-discriminator-training", name="heloc-final-fixed")
    except:
    ...
    improved_trainer = ImprovedDiscriminatorTrainingFinal(hierarchical_discriminators, device)
    ...
    training_history = improved_trainer.alternating_training(
        ...
        resume_from_checkpoint=False  # Disabled due to architecture changes
    )
    ...
    # Generate final impact analysis plot
    logger.info("Generating final impact analysis plot...")
    improved_trainer.plot_impact_analysis_final('heloc_impact_analysis_final.png')
    logger.info("Final impact analysis plot saved to heloc_impact_analysis_final.png")
    ...
    logger.info("Comprehensive results saved to training_results_final.json")
    
    logger.info("Final improved discriminator training completed!")
