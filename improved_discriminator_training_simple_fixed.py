...
from simple_discriminator_fix import fix_discriminators_simple
...
class ImprovedDiscriminatorTrainingSimpleFixed:
    """
    Improved discriminator training with simple, reliable discriminator fixes
    """
    ...
    def __init__(self, hierarchical_discriminators, device="cuda", checkpoint_dir="checkpoints"):
        ...
        self.discriminators_fixed = False
        
        # Create checkpoint directory
        os.makedirs(checkpoint_dir, exist_ok=True)
        
        # Fix discriminators with simple implementations
        self.discriminators_fixed = fix_discriminators_simple(
            self.discriminators, device
        )
        
        if self.discriminators_fixed:
            logger.info("✅ Discriminators successfully fixed with simple implementations!")
        else:
            logger.warning("⚠️ Discriminator fix failed, continuing with original")
    ...
    def save_checkpoint(self, generator, cycle, best_generator_loss, training_history):
        ...
        checkpoint = {
            ...
            'discriminators_fixed': self.discriminators_fixed
        }
    ...
    def load_checkpoint(self, generator, checkpoint_path):
        ...
        if os.path.exists(checkpoint_path):
            ...
            self.discriminators_fixed = checkpoint.get('discriminators_fixed', False)
            
            logger.info(f"📂 Checkpoint loaded from {checkpoint_path}")
            logger.info(f"🔄 Resuming from cycle {checkpoint['cycle']} with best loss {checkpoint['best_generator_loss']:.4f}")
            logger.info(f"🔧 Discriminators fixed status: {'✅ Successful' if self.discriminators_fixed else '❌ Failed'}")
    ...
    def train_discriminators_improved(
        ...
        """Improved discriminator training with fixed discriminators"""
    ...
    def train_sentence_discriminator_batch(self, texts, labels):
        """Train sentence discriminator on a batch with fixed implementation"""
        total_loss = 0
        num_sentences = 0
            ...
            for sentence in sentences:
                if " is " in sentence:
                    try:
                        ...
                        # Use the fixed sentence discriminator
                        prediction = self.discriminators.sentence_discriminator(
                            encoding["input_ids"], encoding.get("attention_mask", None)
                        )
                        ...
                        logger.warning(f"⚠️ Sentence training error: {e}")
                        continue
    ...
    def train_row_discriminator_batch(self, texts, labels):
        """Train row discriminator on a batch with fixed implementation"""
        total_loss = 0
        ...
        for text, label in zip(texts, labels):
            ...
            # Use the fixed row discriminator
    ...
    def train_generator_with_feedback(self, generator, real_data, epochs=2, batch_size=16):
        ...
        # Balanced weights - all discriminators should work now
        balanced_weights = {
            'token': 0.2,
            'sentence': 0.2,  # Now working with simple implementation
            'row': 0.3,       # Now working with simple implementation
    ...
    def plot_impact_analysis_simple_fixed(self, save_path='impact_analysis_simple_fixed.png'):
        """Simple fixed impact analysis plot with comprehensive metrics"""
        ...
        logger.info(f"📊 Simple fixed impact analysis plot saved to {save_path}")
...
def main():
    """Example usage of simple fixed improved discriminator training"""
    # Initialize wandb
    try:
    ...
    # Initialize improved training framework with simple fixed discriminators
    logger.info("🚀 Initializing improved training framework with simple fixed discriminators...")
    improved_trainer = ImprovedDiscriminatorTrainingSimpleFixed(hierarchical_discriminators, device)
    ...
    # Generate simple fixed impact analysis plot
    logger.info("📊 Generating simple fixed impact analysis plot...")
    improved_trainer.plot_impact_analysis_simple_fixed('heloc_impact_analysis_simple_fixed.png')
    logger.info("✅ Simple fixed impact analysis plot saved to heloc_impact_analysis_simple_fixed.png")
    ...
    results = {
        ...
        'training_history': training_history,
        'discriminators_fixed': improved_trainer.discriminators_fixed
    }
    
    with open('training_results_simple_fixed.json', 'w') as f:
        json.dump(results, f, indent=2, default=str)
    logger.info("✅ Comprehensive results saved to training_results_simple_fixed.json")
    
    logger.info("🎉 Simple fixed improved discriminator training completed!")
