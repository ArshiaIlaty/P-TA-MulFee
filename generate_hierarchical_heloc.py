...
def load_hierarchical_discriminators(
    ...
    """
    Load hierarchical discriminators for HELOC with fallback
    """
    try:
        logger.info(f"Loading hierarchical HELOC discriminators from {discriminator_path}")

        if not os.path.exists(discriminator_path):
            logger.warning(f"Discriminator path {discriminator_path} does not exist!")
            logger.info("Creating new hierarchical HELOC discriminators...")

            device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
            discriminators = HierarchicalDiscriminatorSystem(device=device, dataset_type="heloc")
            logger.info("New hierarchical HELOC discriminators created (untrained)")
        ...
        logger.info("Hierarchical HELOC discriminators loaded successfully")
        return discriminators, True  # True indicates trained

    except Exception as e:
        logger.error(f"Error loading hierarchical HELOC discriminators: {str(e)}")
        logger.info("Creating new hierarchical HELOC discriminators as fallback...")
...
def generate_synthetic_data_hierarchical(
    ...
    """
    Generate synthetic HELOC data using hierarchical fine-tuned model with discriminator feedback
    """
    logger.info(
        f"Generating {num_samples} synthetic HELOC samples with hierarchical feedback..."
    )
    ...
    for i in range(num_samples):
        try:
            ...
            if use_discriminators:
                try:
                    ...
                    if quality_score > 0.5:  # Lowered threshold for more samples
                        ...
                        logger.debug(
                            f"HELOC sample {i} accepted with quality score: {quality_score:.3f}"
                        )
                    else:
                        logger.debug(
                            f"HELOC sample {i} rejected with quality score: {quality_score:.3f}"
                        )

                except Exception as e:
                    logger.warning(
                        f"Error getting discriminator feedback for HELOC sample {i}: {str(e)}"
                    )
            ...
            if i % 100 == 0:
                logger.info(
                    f"Generated {i}/{num_samples} HELOC samples, accepted: {len(synthetic_texts)}"
                )
    ...
    logger.info(
        f"Generated {len(synthetic_texts)} synthetic HELOC samples out of {num_samples} attempts"
    )
...
def generate_synthetic_dataset(
    ...
    hierarchical_discriminators,
    device,
    output_file="output_hierarchical_heloc.csv",
    use_discriminators=True,
):
    """
...
def main():
    ...
    """
    try:
        logger.info("=== STARTING HIERARCHICAL HELOC SYNTHETIC DATA GENERATION ===")
        ...
        logger.info("Loading hierarchical HELOC discriminators...")
        hierarchical_discriminators, discriminators_trained = (
            load_hierarchical_discriminators()
        )

        if discriminators_trained:
            logger.info("Using trained hierarchical HELOC discriminators")
            use_discriminators = True
        else:
            logger.warning(
                "Using untrained hierarchical HELOC discriminators - quality filtering disabled"
            )
